...
namespace iMANTRA
{
    public partial class frmCustPriceLst : BaseClass
    {
        ...
        public frmCustPriceLst(BL_BASEFIELD objBL)
        {
        ...
        private void frmCustPriceLst_Load(object sender, EventArgs e)
        {
            dgvCustPriceLst.AutoGenerateColumns = false;
            ...
            foreach (DataGridViewColumn col in dgvCustPriceLst.Columns)
            {
                if (col.Name == "prod_nm")
                {
                ...
                if (col.Name == "rate")
                {
                    dgvCustPriceLst.Columns[col.Name].Tag = "decimal";
                }
                else
                {
                    dgvCustPriceLst.Columns[col.Name].Tag = "string";
                }
            }
        ...
        private void InsertFieldValueToHashTable()
        {
            ...
            int i = 0;
            foreach (DataGridViewRow row in dgvCustPriceLst.Rows)
            {
                ...
                foreach (DataGridViewColumn column in dgvCustPriceLst.Columns)
                {
            ...
            if (dgvCustPriceLst.Rows.Count != 0)
            {
                itserial = int.Parse(dgvCustPriceLst.Rows[--i].Cells["ptserial"].Value.ToString().Trim());
            }
        }
        private void GetFieldValueFromHashTable()
        {
            if (objBASEFILEDS.Tran_id != "0")
            {
                ...
                ClearSearchdgvCustPriceLst(dgvCustPriceLst);
                ...
                foreach (DictionaryEntry entry in objBASEFILEDS.HTITEM)
                {
                    ...
                    dgvCustPriceLst.Rows.Add();
                    foreach (DictionaryEntry entry1 in (Hashtable)entry.Value)
                    {
                        if (dgvCustPriceLst.Columns.Contains(entry1.Key.ToString()))
                        {
                            dgvCustPriceLst.Rows[i].Cells[entry1.Key.ToString()].Value = entry1.Value;
                        }
                    }
                    i++;
                }
                if (dgvCustPriceLst.Rows.Count != 0)
                {
                    itserial = itserial_internal;//int.Parse(dgvCustPriceLst.Rows[--i].Cells["ptserial"].Value.ToString().Trim());
                }
            }
            else
            {
        ...
        public void DisplayControlsonMode(string tran_mode)
        {
            try
            {
                ...
                if (txtSearch.Text != "")
                    BindSearchdgvCustPriceLst(txtSearch.Text);
                else
                    BindSearchdgvCustPriceLst("%");
        ...
        private void frmCustPriceLst_Enter(object sender, EventArgs e)
        {
        ...
        private void frmCustPriceLst_FormClosed(object sender, FormClosedEventArgs e)
        {
            if (((frm_mainmenu)this.MdiParent).ActiveMdiChild != null)
            {
        ...
        private void frmCustPriceLst_Resize(object sender, EventArgs e)
        {
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                dgvCustPriceLst.Rows.Add(1);
                foreach (DataGridViewColumn col in dgvCustPriceLst.Columns)
                {
                    if (col.Visible && (col.DisplayIndex == 1))
                    {
                        dgvCustPriceLst.Rows[dgvCustPriceLst.Rows.Count - 1].Cells[col.Name].Selected = true;
                        dgvCustPriceLst.CurrentCell = dgvCustPriceLst[col.Name, dgvCustPriceLst.Rows.Count - 1];
                        break;
                    }
                }
                dgvCustPriceLst["ptserial", dgvCustPriceLst.Rows.Count - 1].Value = itserial.ToString();
                dgvCustPriceLst["TRAN_CD", dgvCustPriceLst.Rows.Count - 1].Value = objBASEFILEDS.Code;
                ...
                foreach (DictionaryEntry entry in objBASEFILEDS.htitem_details)
                {
                    if (entry.Value != null && entry.Value.ToString().Trim() != "" && dgvCustPriceLst.Columns.Contains(entry.Key.ToString().Trim()))
                    {
                        dgvCustPriceLst[entry.Key.ToString().Trim().ToUpper(), dgvCustPriceLst.Rows.Count - 1].Value = entry.Value;
                    }
                }
            }
            catch (Exception ex)
        ...
        {
            frmPopup objpopup = new frmPopup(objBASEFILEDS.HTMAIN, "cm_mast", "", "ac_id,ac_nm", "ac_nm;Account Name", "Please Select", "tran_cd='CM'", false, "", "0");//,tran_nm;Trasaction Name
        ...
        private void txt_key_down(object sender, KeyEventArgs e)
        {
            try
            {
                ...
                if (e.KeyData == Keys.F2)
                {
                    POPUPTEXTBOX_FOR_GRID dgvCustPriceLstcolumncod = (POPUPTEXTBOX_FOR_GRID)dgvCustPriceLst.Columns[dgvCustPriceLst.CurrentCell.ColumnIndex];
                    frmPopup objfrmPopup = new frmPopup(((Hashtable)objBASEFILEDS.HTITEM[(dgvCustPriceLst.CurrentRow.Cells["PTSERIAL"].Value)]), dgvCustPriceLstcolumncod.Tbl_nm, dgvCustPriceLstcolumncod.Reftbltran_cd, dgvCustPriceLstcolumncod.Primaryddl, dgvCustPriceLstcolumncod.Dispddlfields, "Please select", dgvCustPriceLstcolumncod.Query_con,dgvCustPriceLstcolumncod.IsQcd,dgvCustPriceLstcolumncod.QcdCondition);
                    ...
                    txt.Text = ((Hashtable)objBASEFILEDS.HTITEM[(dgvCustPriceLst.CurrentRow.Cells["PTSERIAL"].Value)])[txt.Name].ToString().Trim();
                    foreach (string str in dgvCustPriceLstcolumncod.Primaryddl.Split(','))
                    {
                        dgvCustPriceLst.CurrentRow.Cells[str].Value = ((Hashtable)objBASEFILEDS.HTITEM[(dgvCustPriceLst.CurrentRow.Cells["PTSERIAL"].Value)])[str];
                    }
                }
            }
            catch (Exception ex)
        ...
        private void dgvCustPriceLst_EditingControlShowing(object sender, DataGridViewEditingControlShowingEventArgs e)
        {
            DataGridView dgvCustPriceLstview = (DataGridView)sender;
            TextBox txt = e.Control as TextBox;
            if (txt != null)
            {
                txt.Name = dgvCustPriceLstview.CurrentCell.OwningColumn.Name.ToString().Trim();
                txt.Tag = dgvCustPriceLstview.CurrentCell.OwningColumn.Tag.ToString().Trim();
                if (txt.Tag.ToString().Trim() == "decimal")
                {
        ...
        private void dgvCustPriceLst_CellValidating(object sender, DataGridViewCellValidatingEventArgs e)
        {
            if (objBASEFILEDS.Tran_mode == "add_mode" || objBASEFILEDS.Tran_mode == "edit_mode")
            {
                dgvCustPriceLst.CommitEdit(DataGridViewDataErrorContexts.Commit);
                if (dgvCustPriceLst != null & e.RowIndex != -1 && e.ColumnIndex != -1)
                {
                    if (((Hashtable)(objBASEFILEDS.HTITEM[(dgvCustPriceLst.CurrentRow.Cells["PTSERIAL"].Value.ToString())])).ContainsKey(dgvCustPriceLst.CurrentCell.OwningColumn.Name))
                    {
                        ((Hashtable)(objBASEFILEDS.HTITEM[(dgvCustPriceLst.CurrentRow.Cells["PTSERIAL"].Value.ToString())]))[dgvCustPriceLst.CurrentCell.OwningColumn.Name] = e.FormattedValue.ToString().Trim();
                    }
                    if (dgvCustPriceLst.CurrentCell.OwningColumn.Name == "prod_nm")
                    {
                        foreach (DictionaryEntry entry in objBASEFILEDS.HTITEM)
                        {
                            if (dgvCustPriceLst.CurrentRow.Cells["PTSERIAL"].Value.ToString() != entry.Key.ToString() && ((Hashtable)entry.Value)["prod_nm"].ToString().ToLower() == e.FormattedValue.ToString().ToLower())
                            {
        ...
        private void dgvCustPriceLst_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            dgvCustPriceLst.CommitEdit(DataGridViewDataErrorContexts.Commit);
        }
        private void dgvCustPriceLst_CellEnter(object sender, DataGridViewCellEventArgs e)
        {
            if (dgvCustPriceLst.CurrentCell.OwningColumn.Name == "prod_nm")
            {
                lblF2.Visible = true;
        ...
        private void dgvCustPriceLst_CellLeave(object sender, DataGridViewCellEventArgs e)
        {
            if (dgvCustPriceLst.CurrentCell.OwningColumn.Name == "prod_nm")
            {
                lblF2.Visible = false;
        ...
        private void dgvCustPriceLst_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
        ...
        private void dgvCustPriceLst_KeyPress(object sender, KeyPressEventArgs e)
        {
            try
            {
        ...
        private bool checkProductValid()
        {
            ...
            else
            {
                ...
                {
                    DataSet dsetCust = objDBAdaper.dsquery("select ac_nm,display_nm from CM_MAST where ac_id='" + objBASEFILEDS.HTMAIN["ac_id"].ToString() + "' and compid='" + objBASEFILEDS.ObjCompany.Compid + "'");
                    if (dsetCust != null && dsetCust.Tables.Count != 0 && dsetCust.Tables[0].Rows.Count != 0)
                    {
                        txtDisp_nm.Text = dsetCust.Tables[0].Rows[0]["display_nm"].ToString();
                    }
                }
            }
            return false;
        ...
        private void btnRemove_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    if (dgvCustPriceLst.CurrentRow.Index != -1)
                    {
                        if (this.tran_mode == "edit_mode")
                        {
                            //if (!objFL_Transaction.Find_Reference(objBASEFILEDS.HTMAIN[objBASEFILEDS.Primary_id.ToString()].ToString(), objBASEFILEDS.HTMAIN["TRAN_NO"].ToString(), objBASEFILEDS.Code, dgvCustPriceLst.Rows[dgvCustPriceLst.CurrentRow.Index].Cells["PTSERIAL"].Value.ToString()))
                            //{
                            //    AutoClosingMessageBox.Show("Sorry! We can't delete item because it is refered in the other transaction","Remove Product",3000);
                            //    return;
                            //}
                        }
                        DialogResult result = MessageBox.Show("Are you sure to delete Product : " + dgvCustPriceLst.CurrentRow.Cells["prod_nm"].Value, "Delete Product", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2);
                        if (result == DialogResult.Yes)
                        {
                            ...
                            {
                                objiDeleteItem.Hashitemvalue = ((Hashtable)objBASEFILEDS.HTITEM[dgvCustPriceLst.CurrentRow.Cells["PTSERIAL"].Value.ToString()]);
                                ...
                                if (validflg)
                                {
                                    ((Hashtable)objBASEFILEDS.HTITEM[dgvCustPriceLst.Rows[dgvCustPriceLst.CurrentRow.Index].Cells["PTSERIAL"].Value.ToString()]).Clear();
                                    foreach (DictionaryEntry entry in objBASEFILEDS.HtPur_Ref)
                                    {
                                        if (entry.Key.ToString().Split(',')[0] == dgvCustPriceLst.Rows[dgvCustPriceLst.CurrentRow.Index].Cells["PTSERIAL"].Value.ToString())
                                        {
                                            objBASEFILEDS.HtPur_Ref.Remove(entry.Key);
                                            break;
                                        }
                                    }
                                    dgvCustPriceLst.Rows.RemoveAt(dgvCustPriceLst.CurrentRow.Index);
                                }
                                if (!validflg)
                                {
                                ...
                                else
                                {
                                    ...
                                    foreach (DictionaryEntry entry in HTITEMVal)
                                    {
                                        ...
                                        {
                                            if (dgvCustPriceLst.CurrentRow.Cells["PTSERIAL"].Value.ToString() == entry.Key.ToString())
                                            {
        ...
        private void ClearSearchdgvCustPriceLst(DataGridView dgv)
        {
        ...
        private void BindSearchdgvCustPriceLst(string strMessage)
        {
            ...
            else
            {
                ...
                ClearSearchdgvCustPriceLst(dgvSearch);
            }
        }
        ...
        private void txtSearch_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == (char)Keys.Enter)
            {
                if (dgvSearch != null)
                {
                    objBASEFILEDS.Tran_id = dgvSearch.CurrentRow.Cells["cplId"].Value.ToString();
                    //AddFieldToHashTable();
                    //GetFieldValueFromHashTable();
        ...
        private void dgvSearch_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (dgvView != null)
            {
                ...
                {
                    objBASEFILEDS.Tran_id = dgvView.CurrentRow.Cells["cplId"].Value.ToString();
                    //AddFieldToHashTable();
                    //GetFieldValueFromHashTable();
                    DisplayControlsonMode("view_mode");
                    // dgvSearch.Focus();
        ...
        private void SelectSearchGrid()
        {
            ...
            if (dgvSearch.CurrentRow != null && objBASEFILEDS.Tran_id != "0")
            {
                ...
                {
                    if (row.Cells["cplId"].Value.ToString().Equals(objBASEFILEDS.Tran_id))
                    {
                        rowIndex = row.Index;
                        // break;
                    }
