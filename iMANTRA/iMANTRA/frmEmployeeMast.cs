...
namespace iMANTRA
{
    public partial class frmEmployeeMast : BaseClass
    {
        ...
        string strSearchField = "emp_code";
        ...
        public frmEmployeeMast(BL_BASEFIELD objBL)
        {
        ...
        {
            objBASEFILEDS.HTMAIN["emp_cd"] = objBASEFILEDS.Tran_id;
            objBASEFILEDS.HTMAIN["emp_code"] = txtEmployee_cd.Text;
            objBASEFILEDS.HTMAIN["fn"] = txtFirst.Text;
            objBASEFILEDS.HTMAIN["mn"] = txtMiddle.Text;
            objBASEFILEDS.HTMAIN["ln"] = txtLast.Text;
            objBASEFILEDS.HTMAIN["contact"] = txtContact.Text;
            objBASEFILEDS.HTMAIN["email"] = txtEmail.Text;
            objBASEFILEDS.HTMAIN["desgn"] = txtDesg.Text;
            objBASEFILEDS.HTMAIN["doj"] = dtpDoj.DtValue.ToString("yyyy/MM/dd"); 
            objBASEFILEDS.HTMAIN["dot"] = dtpDot.DtValue.ToString("yyyy/MM/dd"); 
        ...
        {
            txtEmployee_cd.Text = objBASEFILEDS.HTMAIN["emp_code"].ToString();
            txtFirst.Text = objBASEFILEDS.HTMAIN["fn"].ToString();
            txtMiddle.Text = objBASEFILEDS.HTMAIN["mn"].ToString();
            txtLast.Text = objBASEFILEDS.HTMAIN["ln"].ToString();
            txtContact.Text = objBASEFILEDS.HTMAIN["contact"].ToString();
            txtEmail.Text = objBASEFILEDS.HTMAIN["email"].ToString();
            txtDesg.Text = objBASEFILEDS.HTMAIN["desgn"].ToString();
            dtpDoj.DtValue = objBASEFILEDS.HTMAIN["doj"] != null && objBASEFILEDS.HTMAIN["doj"].ToString() != "" ? DateTime.Parse(objBASEFILEDS.HTMAIN["doj"].ToString()) : DateTime.Parse("01/01/1900");
            dtpDot.DtValue = objBASEFILEDS.HTMAIN["dot"] != null && objBASEFILEDS.HTMAIN["dot"].ToString() != "" ? DateTime.Parse(objBASEFILEDS.HTMAIN["dot"].ToString()) : DateTime.Parse("01/01/1900");
        ...
        private bool Click_Save()
        {
            ...
            if (txtEmployee_cd.Text == "") { AutoClosingMessageBox.Show("Please enter valid Employee Code.","Validation",3000); flg = false; }
            else if (txtFirst.Text == "") { AutoClosingMessageBox.Show("Please enter valid First Name.","Validation",3000); flg = false; }
            return flg;
        }
            ...
            try
            {
                ...
                switch (tran_mode)
                {
                    case "add_mode":
                        ...
                        txtEmployee_cd.Focus();
                        break;
                    case "edit_mode":
                        AddFieldToHashTable();
                        //BindControls();
                        txtEmployee_cd.Enabled = false;
        ...
        private void frmEmployeeMast_Load(object sender, EventArgs e)
        {
            //BindControls();
        ...
        private void frmEmployeeMast_Enter(object sender, EventArgs e)
        {
        ...
        private void frmEmployeeMast_FormClosed(object sender, FormClosedEventArgs e)
        {
            if (((frm_mainmenu)this.MdiParent).ActiveMdiChild != null)
            {
        ...
        private void frmEmployeeMast_Resize(object sender, EventArgs e)
        {
        ...
        private void OTHER_DET_Click(object sender, EventArgs e)
        {
            ...
            //try
            //{
            //    SmtpMail.SmtpServer.Insert(0, "INODE-SHA");
            //    MailMessage Msg = new MailMessage();
            //    Msg.To = "sharanamma.jekeen@inodetechnologies.com";
            //    Msg.From = "sharanamma.jekeen@inodetechnologies.com";
            //    Msg.Subject = "Crystal Report Attachment ";
            //    Msg.Body = "Crystal Report Attachment ";
            //    Msg.Attachments.Add(new MailAttachment("D:\\TOP25.xlsx"));
        ...
        private void txtEmployee_cd_Validating(object sender, CancelEventArgs e)
        {
            if (objBASEFILEDS.Tran_mode != "view_mode")
            {
                if (txtEmployee_cd.Text == "")
                {
                    AutoClosingMessageBox.Show("Please enter valid Employee Code","Validation",3000);
                    e.Cancel = true;
                }
                else
                {
                    DataSet ds = objDBAdaper.dsquery("select emp_cd,emp_code,compid from " + objBASEFILEDS.Main_tbl_nm + " where emp_code ='" + txtEmployee_cd.Text.Replace("'", "''") + "' and emp_cd!='" + objBASEFILEDS.Tran_id + "' and compid='" + objBASEFILEDS.ObjCompany.Compid + "'");
                    if (ds != null && ds.Tables.Count != 0 && ds.Tables[0].Rows.Count != 0)
                    {
                        AutoClosingMessageBox.Show("Employee Code already exists","Validation",3000);
                        e.Cancel = true;
                    }
                }
            }
        ...
        private void txtFirst_Validating(object sender, CancelEventArgs e)
        {
            if (objBASEFILEDS.Tran_mode != "view_mode")
            {
                if (txtFirst.Text == "")
                {
                    AutoClosingMessageBox.Show("Please enter First Name","Validation",3000);
                    e.Cancel = true;
                }
            }
        }
        ...
        {
            string strQuery = "select emp_cd,emp_code,fn,ln from " + objBASEFILEDS.Main_tbl_nm + " where " + strSearchField + " like '%" + strMessage.Replace("'","''") + "%' and compid='" + objBASEFILEDS.ObjCompany.Compid.ToString() + "'";
        ...
        private void txtSearch_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == (char)Keys.Enter)
            {
                if (dgvSearch != null)
                {
                    objBASEFILEDS.Tran_id = dgvSearch.CurrentRow.Cells["emp_cd"].Value.ToString();
                    //AddFieldToHashTable();
                    //GetFieldValueFromHashTable();
        ...
        private void dgvSearch_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                objBASEFILEDS.Tran_id = dgvView.CurrentRow.Cells["emp_cd"].Value.ToString();
                AddFieldToHashTable();
                GetFieldValueFromHashTable();
                SelectSearchGrid();
            }
        ...
        private void SelectSearchGrid()
        {
            ...
            if (dgvSearch.CurrentRow != null && objBASEFILEDS.Tran_id != "0")
            {
                ...
                {
                    if (row.Cells["emp_cd"].Value.ToString().Equals(objBASEFILEDS.Tran_id))
                    {
                        rowIndex = row.Index;
                        // break;
                    }
