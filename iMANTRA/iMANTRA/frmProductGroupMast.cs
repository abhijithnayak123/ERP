...
namespace iMANTRA
{
    public partial class frmProductGroupMast : BaseClass
    {
        ...
        string strSearchField = "pt_grp_nm";
        ...
        public frmProductGroupMast(BL_BASEFIELD objBL)
        {
        ...
        {
            objBASEFILEDS.HTMAIN["pt_grp_id"] = objBASEFILEDS.Tran_id;

            objBASEFILEDS.HTMAIN["Pt_grp_nm"] = txtProd_gr.Text;
            objBASEFILEDS.HTMAIN["prod_desc"] = txtProdGrDesc.Text;
            objBASEFILEDS.HTMAIN["PROD_TY_NM"] = cmProd_Type.Text;
            objBASEFILEDS.HTMAIN["prod_tyid"] = cmProd_Type.SelectedValue;
            objBASEFILEDS.HTMAIN["uom"] = cmbUOM.SelectedValue;
        ...
        private void GetFieldValueFromHashTable()
        {
            ...
            txtProdGrDesc.Text = objBASEFILEDS.HTMAIN["prod_desc"].ToString();
            cmProd_Type.Text = objBASEFILEDS.HTMAIN["PROD_TY_NM"].ToString();
            cmbUOM.Text = objBASEFILEDS.HTMAIN["uom"].ToString();
        ...
        private bool Click_Save()
        {
            ...
            if (txtProd_gr.Text == "") { AutoClosingMessageBox.Show("Please enter valid Product Group Code.", "Validation", 3000); flg = false; }
        ...
        private void txtProd_gr_Validating(object sender, CancelEventArgs e)
        {
            if (objBASEFILEDS.Tran_mode != "view_mode")
            {
                if (txtProd_gr.Text == "")
                {
                    AutoClosingMessageBox.Show("Please enter valid Product Group Code", "Validation", 3000);
                    e.Cancel = true;
                }
                else
                {
                    DataSet ds = objDBAdaper.dsquery("select pt_grp_id,pt_grp_nm from " + objBASEFILEDS.Main_tbl_nm + " where pt_grp_nm ='" + txtProd_gr.Text.Replace("'", "''") + "' and pt_grp_id!='" + objBASEFILEDS.Tran_id + "' and compid='" + objBASEFILEDS.ObjCompany.Compid + "'");
                    if (ds != null && ds.Tables.Count != 0 && ds.Tables[0].Rows.Count != 0)
                    {
                        AutoClosingMessageBox.Show("Product Group Code already exists", "Validation", 3000);
                        e.Cancel = true;
                    }
                }
            }
        ...
        private void cmbUOM_Validating(object sender, CancelEventArgs e)
        {
            if (objBASEFILEDS.Tran_mode != "view_mode")
            {
        ...
        {
            string strQuery = "select pt_grp_id,pt_grp_nm,prod_desc from " + objBASEFILEDS.Main_tbl_nm + " where " + strSearchField + " like '%" + strMessage.Replace("'","''") + "%' and compid='" + objBASEFILEDS.ObjCompany.Compid.ToString() + "'";
        ...
        private void txtSearch_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == (char)Keys.Enter)
            {
                if (dgvSearch != null)
                {
                    objBASEFILEDS.Tran_id = dgvSearch.CurrentRow.Cells["pt_grp_id"].Value.ToString();
                    //AddFieldToHashTable();
                    //GetFieldValueFromHashTable();
        ...
        private void dgvSearch_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                objBASEFILEDS.Tran_id = dgvView.CurrentRow.Cells["pt_grp_id"].Value.ToString();
                AddFieldToHashTable();
                GetFieldValueFromHashTable();
                SelectSearchGrid();
            }
        ...
        private void SelectSearchGrid()
        {
            ...
            if (dgvSearch.CurrentRow != null && objBASEFILEDS.Tran_id != "0")
            {
                ...
                {
                    if (row.Cells["pt_grp_id"].Value.ToString().Equals(objBASEFILEDS.Tran_id))
                    {
                        rowIndex = row.Index;
                        // break;
                    }
