...
namespace iMANTRA
{
    public partial class frmProcessMaster : BaseClass
    {
        ...
        public frmProcessMaster(BL_BASEFIELD objBL)
        {
        ...
        private void frmProcessMaster_Load(object sender, EventArgs e)
        {
            ...
            foreach (DataGridViewColumn col in dgvProcess.Columns)
            {
                if (col.Name == "mac_type")
                {
                    POPUPTEXTBOX_FOR_GRID txtcol = (POPUPTEXTBOX_FOR_GRID)col;
                    txtcol.Dispddlfields = "mac_type;Machine Type";
                    txtcol.Primaryddl = "mac_type_id,mac_type";
                    txtcol.Query_con = "";
                    txtcol.Tbl_nm = "MACHINE_TYPE_MAST";
                }
                if (col.Name == "opt_type")
                {
                    POPUPTEXTBOX_FOR_GRID txtcol = (POPUPTEXTBOX_FOR_GRID)col;
                    txtcol.Dispddlfields = "opt_type;Operation Type";
                    txtcol.Primaryddl = "opt_id,opt_type";
                    txtcol.Query_con = "";
                    txtcol.Tbl_nm = "OPERATION_TYPE";
                }
                if (col.Name == "pro_cycle" || col.Name == "pro_setup" || col.Name == "pro_cost")
                {
                    dgvProcess.Columns[col.Name].Tag = "decimal";
                }
                else
                {
                    dgvProcess.Columns[col.Name].Tag = "string";
                }
            }
        ...
        private void AddFieldToHashTable()
        {
            ...
            objBASEFILEDS.dsetview = objDBAdaper.dsquery("select * from  " + objBASEFILEDS.Item_tbl_nm + " where " + objBASEFILEDS.Primary_id + "=" + objBASEFILEDS.Tran_id + " and compid='" + objBASEFILEDS.ObjCompany.Compid.ToString() + "' order by " + objBASEFILEDS.Primary_id + ",pro_order");
            foreach (DataRow row in objBASEFILEDS.dsetview.Tables[0].Rows)
            {
        ...
        {
            objBASEFILEDS.HTMAIN["pro_id"] = objBASEFILEDS.Tran_id;
            objBASEFILEDS.HTMAIN["prod_nm"] = txtProduct.Text;
            objBASEFILEDS.HTMAIN["prod_desc"] = txtDesc.Text;
            objBASEFILEDS.HTMAIN["pro_remark"] = txtRemarks.Text;
            ...
            foreach (DataGridViewRow row in dgvProcess.Rows)
            {
                ...
                foreach (DataGridViewColumn column in dgvProcess.Columns)
                {
            ...
            {
                itserial = int.Parse(dgvProcess.Rows[--i].Cells["pro_order"].Value.ToString().Trim());
            }
        }
        private void GetFieldValueFromHashTable()
        {
            if (objBASEFILEDS.Tran_id != "0")
            {
                ...
                txtDesc.Text = objBASEFILEDS.HTMAIN["prod_desc"].ToString();
                txtRemarks.Text = objBASEFILEDS.HTMAIN["pro_remark"].ToString();
                ClearSearchdgvProcess(dgvProcess);
                ...
                foreach (DictionaryEntry entry in objBASEFILEDS.HTITEM)
                {
                    ...
                    {
                        if (dgvProcess.Columns.Contains(entry1.Key.ToString()))
                        {
                            dgvProcess.Rows[i].Cells[entry1.Key.ToString()].Value = entry1.Value;
                        }
                    }
                    i++;
                }
        ...
        private void frmProcessMaster_Enter(object sender, EventArgs e)
        {
        ...
        private void frmProcessMaster_FormClosed(object sender, FormClosedEventArgs e)
        {
            if (((frm_mainmenu)this.MdiParent).ActiveMdiChild != null)
            {
        ...
        private void frmProcessMaster_Resize(object sender, EventArgs e)
        {
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                dgvProcess.Rows.Add(1);
                foreach (DataGridViewColumn col in dgvProcess.Columns)
                {
                    if (col.Visible && (col.DisplayIndex == 1))
                    {
                        dgvProcess.Rows[dgvProcess.Rows.Count - 1].Cells[col.Name].Selected = true;
                        dgvProcess.CurrentCell = dgvProcess[col.Name, dgvProcess.Rows.Count - 1];
                        break;
                    }
                }

                dgvProcess["pro_order", dgvProcess.Rows.Count - 1].Value = itserial.ToString();
                dgvProcess["ptserial", dgvProcess.Rows.Count - 1].Value = itserial.ToString();
                dgvProcess["TRAN_CD", dgvProcess.Rows.Count - 1].Value = objBASEFILEDS.Code;
                ((Hashtable)objBASEFILEDS.HTITEM[itserial.ToString()])["ptserial"] = itserial.ToString();
                ((Hashtable)objBASEFILEDS.HTITEM[itserial.ToString()])["pro_order"] = itserial.ToString();
                ((Hashtable)objBASEFILEDS.HTITEM[itserial.ToString()])["TRAN_CD"] = objBASEFILEDS.Code;
                ((Hashtable)objBASEFILEDS.HTITEM[itserial.ToString()])["pro_order_id"] = "0";

                foreach (DictionaryEntry entry in objBASEFILEDS.htitem_details)
                {
                    if (entry.Value != null && entry.Value.ToString().Trim() != "" && dgvProcess.Columns.Contains(entry.Key.ToString().Trim()))
                    {
                        dgvProcess[entry.Key.ToString().Trim().ToUpper(), dgvProcess.Rows.Count - 1].Value = entry.Value;
                    }
                }
            }
            catch (Exception ex)
        ...
        {
            if (dgvProcess.CurrentCell.OwningColumn.Name == "opt_type" || dgvProcess.CurrentCell.OwningColumn.Name == "mac_type")
            {
                lblF2.Visible = true;
            }
        }
        ...
        private void dgvProcess_CellValidating(object sender, DataGridViewCellValidatingEventArgs e)
        {
            if (objBASEFILEDS.Tran_mode == "add_mode" || objBASEFILEDS.Tran_mode == "edit_mode")
            {
                ...
                if (dgvProcess != null & e.RowIndex != -1 && e.ColumnIndex != -1)
                {
                    if (((Hashtable)(objBASEFILEDS.HTITEM[(dgvProcess.CurrentRow.Cells["PTSERIAL"].Value.ToString())])).ContainsKey(dgvProcess.CurrentCell.OwningColumn.Name))
                    {
                        if ((dgvProcess.CurrentCell.OwningColumn.Name == "pro_order") || (dgvProcess.CurrentRow.Cells["pro_nm"].Value != null && dgvProcess.CurrentRow.Cells["pro_nm"].Value.ToString() != ""))
                        {
                            ((Hashtable)(objBASEFILEDS.HTITEM[(dgvProcess.CurrentRow.Cells["PTSERIAL"].Value.ToString())]))[dgvProcess.CurrentCell.OwningColumn.Name] = e.FormattedValue.ToString().Trim();
                        }
                    }
                }
            }
        ...
        {
            using (SolidBrush b = new SolidBrush(dgvProcess.RowHeadersDefaultCellStyle.ForeColor))
            {
        ...
        private void dgvProcess_KeyPress(object sender, KeyPressEventArgs e)
        {
            try
            {
        ...
        private void txt_key_down(object sender, KeyEventArgs e)
        {
            try
            {
                ...
                if (e.KeyData == Keys.F2)
                {
                    POPUPTEXTBOX_FOR_GRID dgvProcesscolumncod = (POPUPTEXTBOX_FOR_GRID)dgvProcess.Columns[dgvProcess.CurrentCell.ColumnIndex];
                    frmPopup objfrmPopup = new frmPopup(((Hashtable)objBASEFILEDS.HTITEM[(dgvProcess.CurrentRow.Cells["PTSERIAL"].Value)]), dgvProcesscolumncod.Tbl_nm, dgvProcesscolumncod.Reftbltran_cd, dgvProcesscolumncod.Primaryddl, dgvProcesscolumncod.Dispddlfields, "Please select", dgvProcesscolumncod.Query_con, dgvProcesscolumncod.IsQcd, dgvProcesscolumncod.QcdCondition);
                    ...
                    txt.Text = ((Hashtable)objBASEFILEDS.HTITEM[(dgvProcess.CurrentRow.Cells["PTSERIAL"].Value)])[txt.Name].ToString().Trim();
                    foreach (string str in dgvProcesscolumncod.Primaryddl.Split(','))
                    {
                        dgvProcess.CurrentRow.Cells[str].Value = ((Hashtable)objBASEFILEDS.HTITEM[(dgvProcess.CurrentRow.Cells["PTSERIAL"].Value)])[str];
                    }
                }
            }
            catch (Exception ex)
        ...
        private bool checkProductValid()
        {
            ...
            {
                DataSet ds = objDBAdaper.dsquery("select prod_cd,prod_nm from PMMAIN where prod_nm ='" + txtProduct.Text.Replace("'", "''") + "' and " + objBASEFILEDS.Primary_id + "!='" + objBASEFILEDS.Tran_id + "' and compid='" + objBASEFILEDS.ObjCompany.Compid + "'");
        ...
        private void btnRemove_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    if (dgvProcess.CurrentRow.Index != -1)
                    {
                        ...
                        DialogResult result = MessageBox.Show("Are you sure to delete Process : " + dgvProcess.CurrentRow.Cells["pro_nm"].Value, "Delete Process", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2);
                        if (result == DialogResult.Yes)
                        {
                            ...
                            {
                                objiDeleteItem.Hashitemvalue = ((Hashtable)objBASEFILEDS.HTITEM[dgvProcess.CurrentRow.Cells["PTSERIAL"].Value.ToString()]);
                                ...
                                if (validflg)
                                {
                                    ((Hashtable)objBASEFILEDS.HTITEM[dgvProcess.Rows[dgvProcess.CurrentRow.Index].Cells["PTSERIAL"].Value.ToString()]).Clear();
                                    foreach (DictionaryEntry entry in objBASEFILEDS.HtPur_Ref)
                                    {
                                        if (entry.Key.ToString().Split(',')[0] == dgvProcess.Rows[dgvProcess.CurrentRow.Index].Cells["PTSERIAL"].Value.ToString())
                                        {
                                            objBASEFILEDS.HtPur_Ref.Remove(entry.Key);
                                            break;
                                        }
                                    }
                                    dgvProcess.Rows.RemoveAt(dgvProcess.CurrentRow.Index);
                                }
                                if (!validflg)
                                {
                                ...
                                else
                                {
                                    ...
                                    foreach (DictionaryEntry entry in HTITEMVal)
                                    {
                                        ...
                                        {
                                            if (dgvProcess.CurrentRow.Cells["PTSERIAL"].Value.ToString() == entry.Key.ToString())
                                            {
        ...
        {
            string strQuery = "select pro_id,prod_nm,prod_desc from " + objBASEFILEDS.Main_tbl_nm + " where " + strSearchField + " like '%" + strMessage.Replace("'","''") + "%' and compid='" + objBASEFILEDS.ObjCompany.Compid.ToString() + "'";
        ...
        private void txtSearch_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == (char)Keys.Enter)
            {
                if (dgvSearch != null)
                {
                    objBASEFILEDS.Tran_id = dgvSearch.CurrentRow.Cells["pro_id"].Value.ToString();
                    //AddFieldToHashTable();
                    //GetFieldValueFromHashTable();
        ...
        private void dgvSearch_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                objBASEFILEDS.Tran_id = dgvView.CurrentRow.Cells["pro_id"].Value.ToString();
                AddFieldToHashTable();
                GetFieldValueFromHashTable();
                SelectSearchGrid();
            }
        ...
        private void SelectSearchGrid()
        {
            ...
            if (dgvSearch.CurrentRow != null && objBASEFILEDS.Tran_id != "0")
            {
                ...
                {
                    if (row.Cells["pro_id"].Value.ToString().Equals(objBASEFILEDS.Tran_id))
                    {
                        rowIndex = row.Index;
                        // break;
                    }
